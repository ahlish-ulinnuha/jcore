-- Rollback for supabase/master_products_from_excel.sql
-- Safe-ish rollback: removes mappings from the Excel source first, then removes products only if they are not used by requests and no longer have vendor mappings, then removes unused brands.
...
with source_data(product_name, brand_name, vendor_id) as (
  ...
  select distinct p.id as product_id, b.id as brand_id
  from normalized n
  join public.brands b on b.name = n.brand_name
  ...
  delete from public.product_vendors pv
  using normalized n, target_products tp
  where pv.product_id = tp.product_id
    and pv.vendor_id = n.vendor_id
  returning pv.product_id
), deleted_products as (
  delete from public.products p
  using target_products tp
  where p.id = tp.product_id
    and not exists (select 1 from public.purchase_request_items pri where pri.product_id = p.id)
    and not exists (select 1 from public.product_vendors pv where pv.product_id = p.id)
  returning p.id, p.brand_id
)
delete from public.brands b
where b.name in (select distinct brand_name from normalized)
  and not exists (select 1 from public.products p where p.brand_id = b.id);

-- Check leftovers from this Excel source after rollback.
with source_data(product_name, brand_name, vendor_id) as (
  values
    ('fish tofu mini', 'GIZI', 'd5b38434-75c1-46b6-bcfc-c934597f29ec'),
    ('baso salmon', 'GIZI', 'd5b38434-75c1-46b6-bcfc-c934597f29ec'),
  ...
  select trim(product_name) as product_name, upper(trim(brand_name)) as brand_name
  from source_data
)
select p.id, p.name, b.name as brand_name
from normalized n
join public.brands b on b.name = n.brand_name
join public.products p on lower(trim(p.name)) = lower(trim(n.product_name)) and p.brand_id = b.id;
