...
function renderStores({ active, baseParams, currentPage, pageSize, q, storeRows }: { active: string; baseParams: URLSearchParams; currentPage: number; pageSize: number; q: string; storeRows: Store[] }) {
  ...
  return (
    <>
      <form className="panel form master-add-form" action={createStore}><h2>Tambah Store</h2><div className="filter-grid"><div className="field"><label>Kode</label><input name="code" placeholder="J1" /></div><div className="field"><label>Nama Store</label><input name="name" required placeholder="J1" /></div><div className="field"><label>Latitude</label><input name="latitude" placeholder="-6.200000" type="text" /></div><div className="field"><label>Longitude</label><input name="longitude" placeholder="106.816666" type="text" /></div><div className="field"><label>Radius Absensi (meter)</label><input defaultValue={150} name="geofence_radius_m" type="number" /></div><MasterSubmitButton label="Tambah Store" pendingLabel="Menambah..." variant="primary" /></div></form>
      <section className="panel"><MasterFilter active={active} pageSize={pageSize} q={q} roleFilter="all" section="store" storeFilter="all" storeRows={storeRows} /><div className="table-wrap"><table><thead><tr><th>Kode</th><th>Store</th><th>Latitude</th><th>Longitude</th><th>Radius (m)</th><th>Aktif</th><th>Aksi</th></tr></thead><tbody>{page.rows.map((store) => (
        <tr key={store.id}><td><form id={`store-${store.id}`} action={updateStore} className="inline-edit-form"><input name="id" type="hidden" value={store.id} /><input name="code" defaultValue={store.code ?? ""} /></form></td><td><input name="name" form={`store-${store.id}`} defaultValue={store.name} required /></td><td><input defaultValue={store.latitude ?? ""} form={`store-${store.id}`} name="latitude" type="text" /></td><td><input defaultValue={store.longitude ?? ""} form={`store-${store.id}`} name="longitude" type="text" /></td><td><input defaultValue={store.geofence_radius_m} form={`store-${store.id}`} name="geofence_radius_m" type="number" /></td><td><input name="is_active" form={`store-${store.id}`} type="checkbox" defaultChecked={store.is_active} /></td><td><div className="row-actions"><MasterSubmitButton form={`store-${store.id}`} label="Edit" pendingLabel="Menyimpan..." /><form action={deleteStore}><input name="id" type="hidden" value={store.id} /><MasterSubmitButton label="Hapus" pendingLabel="Menghapus..." variant="danger" /></form></div></td></tr>
...
function renderUsers({ active, baseParams, currentPage, menuAccessRows, pageSize, profileRows, q, roleFilter, storeFilter, storeRows }: { active: string; baseParams: URLSearchParams; currentPage: number; menuAccessRows: ProfileMenuAccess[]; pageSize: number; profileRows: Profile[]; q: string; roleFilter: string; storeFilter: string; storeRows: Store[] }) {
  ...
  return (
    <>
      <form className="panel form master-add-form" action={upsertProfile}><h2>Tambah User</h2><p className="muted">Buat email/password user dulu di Supabase Authentication, lalu paste User UID di sini.</p><div className="filter-grid"><div className="field"><label>User UID</label><input name="id" required /></div><div className="field"><label>Email</label><input name="email" type="email" required /></div><div className="field"><label>Nama</label><input name="full_name" required /></div><div className="field"><label>Role</label><select name="role" defaultValue="staff"><option value="admin">admin</option><option value="staff">staff</option><option value="vendor">vendor</option></select></div><div className="field"><label>Store</label><select name="store_id"><option value="">Tidak ada</option>{storeRows.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select></div><div className="field"><label>Slack Member ID</label><input name="slack_member_id" placeholder="U0123456789" /></div><div className="field"><label>No. WhatsApp</label><input name="phone" placeholder="08123456789" /></div><MasterSubmitButton label="Tambah User" pendingLabel="Menambah..." variant="primary" /></div><h3>Akses Menu</h3><MenuAccessFields checkedKeys={defaultNewUserMenuKeys} /></form>
      <section className="panel"><MasterFilter active={active} pageSize={pageSize} q={q} roleFilter={roleFilter} section="user" storeFilter={storeFilter} storeRows={storeRows} /><form action={resetAllProfilePasswords} className="table-toolbar"><MasterSubmitButton label="Reset Password Semua User" pendingLabel="Mereset..." /></form><div className="table-wrap"><table><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Store</th><th>Slack Member ID</th><th>No. WhatsApp</th><th>Akses Menu</th><th>User ID</th><th>Aksi</th></tr></thead><tbody>{page.rows.map((row) => {
        const formId = `profile-${row.id}`;
        const checkedKeys = menuKeysForProfileAccess(row.id, row.role, menuAccessRows);
        return (
        <tr key={row.id}><td><form id={formId} action={upsertProfile} className="inline-edit-form"><input name="id" type="hidden" value={row.id} /><input name="full_name" defaultValue={row.full_name} required /></form></td><td><input name="email" form={formId} defaultValue={row.email ?? ""} type="email" /></td><td><select name="role" form={formId} defaultValue={row.role}><option value="admin">admin</option><option value="staff">staff</option><option value="vendor">vendor</option></select></td><td><select name="store_id" form={formId} defaultValue={row.store_id ?? ""}><option value="">Tidak ada</option>{storeRows.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select></td><td><input name="slack_member_id" form={formId} defaultValue={row.slack_member_id ?? ""} placeholder="U0123456789" /></td><td><input name="phone" form={formId} defaultValue={row.phone ?? ""} placeholder="08123456789" /></td><td><form action={updateProfileMenuAccess} className="menu-access-form"><input name="profile_id" type="hidden" value={row.id} /><MenuAccessFields checkedKeys={checkedKeys} /><MasterSubmitButton label="Simpan Akses" pendingLabel="Menyimpan..." /></form></td><td>{row.id}</td><td><div className="row-actions"><MasterSubmitButton form={formId} label="Edit" pendingLabel="Menyimpan..." /><form action={resetProfilePassword}><input name="email" type="hidden" value={row.email ?? ""} /><MasterSubmitButton disabled={!row.email} label="Reset Password" pendingLabel="Mereset..." title="Reset password" /></form><form action={deleteProfile}><input name="id" type="hidden" value={row.id} /><MasterSubmitButton label="Hapus" pendingLabel="Menghapus..." variant="danger" /></form></div></td></tr>
      );})}</tbody></table></div><Pagination currentPage={page.currentPage} params={baseParams} totalPages={page.totalPages} /></section>
    </>
  );
}
